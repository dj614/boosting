...
def _build_classification_split(
    X: Array,
    beta_true: Array,
    rng: np.random.Generator,
    feature_mean: Array,
    ...
    raw_signal = X @ beta_true
    signal_std = float(np.std(raw_signal))
    scale = float(logit_scale) / max(signal_std, 1e-8)
    scaled_signal = raw_signal * scale
    prob = _sigmoid(scaled_signal)
    y = rng.binomial(1, prob, size=X.shape[0]).astype(float)

    corr_to_signal = np.corrcoef(X, scaled_signal[:, None], rowvar=False)[-1, :-1]
    corr_to_signal = np.nan_to_num(corr_to_signal, nan=0.0)

    meta: Dict[str, Array] = {
        "feature_mean": feature_mean.astype(float),
        "feature_std": feature_std.astype(float),
        "signal": scaled_signal.astype(float),
        "raw_signal": raw_signal.astype(float),
        "true_prob": prob.astype(float),
        "logit_scale": np.full(X.shape[0], float(logit_scale), dtype=float),
        "active_mask": np.isin(np.arange(X.shape[1]), support_true).astype(int),
        "corr_to_signal": corr_to_signal.astype(float),
        "design_id": np.full(X.shape[0], ["independent", "block_correlated", "strong_collinear"].index(design), dtype=int),
    }
    return SparseRegressionSplit(X=X.astype(float), y=y.astype(float), signal=scaled_signal.astype(float), meta=meta)
...
def generate_sparse_classification_dataset(
    ...
    train = _build_classification_split(
        X=X_train,
        beta_true=beta_true,
        rng=rng,
    ...
    valid = _build_classification_split(
        X=X_valid,
        beta_true=beta_true,
        rng=rng,
        feature_mean=feature_mean,
    ...
    test = _build_classification_split(
        X=X_test,
        beta_true=beta_true,
        rng=rng,
        feature_mean=feature_mean,
...
def summarize_sparse_classification_dataset(dataset: SparseRegressionDataset) -> Dict[str, float]:
    active_idx = dataset.support_true
    inactive_mask = np.ones(dataset.beta_true.shape[0], dtype=bool)
    inactive_mask[active_idx] = False

    train_signal = dataset.train.signal
    train_prob = np.asarray(dataset.train.meta.get("true_prob"), dtype=float)
    sigma_hat = np.corrcoef(dataset.train.X, rowvar=False)
    off_diag = sigma_hat[~np.eye(sigma_hat.shape[0], dtype=bool)]
    y_train = np.asarray(dataset.train.y, dtype=float)
    ...
    summary: Dict[str, float] = {
        ...
        "train_signal_mean": float(train_signal.mean()),
        "train_signal_std": float(train_signal.std()),
        "train_prob_mean": float(train_prob.mean()),
        "train_prob_std": float(train_prob.std()),
        "train_positive_rate": float(y_train.mean()),
        "train_label_entropy": float(-(np.clip(y_train.mean(), 1e-8, 1.0 - 1e-8) * np.log(np.clip(y_train.mean(), 1e-8, 1.0 - 1e-8)) + (1.0 - np.clip(y_train.mean(), 1e-8, 1.0 - 1e-8)) * np.log(np.clip(1.0 - y_train.mean(), 1e-8, 1.0 - 1e-8)))),
        "active_abs_beta_mean": float(np.mean(np.abs(dataset.beta_true[active_idx]))),
        "inactive_abs_beta_mean": float(np.mean(np.abs(dataset.beta_true[inactive_mask]))),
        "train_feature_std_mean": float(dataset.train.X.std(axis=0).mean()),
        "train_feature_std_min": float(dataset.train.X.std(axis=0).min()),
